...
function BulkSchemeCard({
    ...
    const orderRange =
        scheme.maxAmount === Infinity
            ? `₹${scheme.minAmount.toLocaleString("en-IN")} and Above`
            : `₹${scheme.minAmount.toLocaleString("en-IN")} - ₹${scheme.maxAmount.toLocaleString("en-IN")}`;
    ...
    return (
        ...
        >
            ...
            {/* Footer */}
            <div className="mt-5 flex flex-col gap-3 pr-4 sm:pr-0 sm:flex-row sm:items-center sm:justify-between">
                <span className="inline-flex w-fit rounded-full bg-primary/10 px-3 py-1 text-sm font-medium text-primary">
                    Bulk Pricing
                </span>
